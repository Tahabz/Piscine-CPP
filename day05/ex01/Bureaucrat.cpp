...
Bureaucrat::Bureaucrat(const std::string &name, int grade) throw(Bureaucrat::GradeTooHighException,
																 Bureaucrat::GradeTooLowException) : _name(name)
{
	if (grade < 1)
...
void Bureaucrat::signForm(Form &form) const
{
	...
	{
		std::cout << this->getName() << " could not sign the form because " << e.what() << std::endl;
	}
}
